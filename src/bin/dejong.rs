//! Peter de Jong attractor visualization on LED matrix.
...
use embassy_adafruit_rpi_2040_uf2_led_matrix::dejong::DeJong;
...
static DEJONG: StaticCell<DeJong> = StaticCell::new();
...
async fn graphics_task() {
    ...
    let dejong = DEJONG.init(DeJong::new());

    defmt::info!("Starting de Jong simulation");

    loop {
        inactive_lmd.clear();
        dejong.draw(inactive_lmd).unwrap();
